...
Covers: players, assets, locations, features, ... PLUS (new) Projects & Tasks.

Safe to import in later phases; not required for Phase 1 run unless you use
the Projects/Tasks features.
"""
...
# NEW: task status for player guide
...
class Player(SQLModel, table=True):
    ...
    # Relationships
    deeds: List["Deed"] = Relationship(back_populates="owner")
...
class StarSystem(SQLModel, table=True):
    ...
    locations: List["Location"] = Relationship(back_populates="system")
...
class LocationAssessment(SQLModel, table=True):
    ...
    summary: str = ""
    details_text: str = ""
    data: Dict = Field(default_factory=dict, sa_column=Column(JSON) if JSON else None)
...
# -----------------------------
# Convenience/seed for Projects & Tasks
# -----------------------------
def seed_projects_if_empty(session: Session) -> None:
    ...
    def add_task(p: Project, key: str, name: str, desc: str, idx: int, status=TaskStatus.UNASSIGNED, hidden=True):
        t = Task(project_id=p.id, key=key, name=name, description=desc, order_index=idx, status=status, hidden=hidden)
        session.add(t)
